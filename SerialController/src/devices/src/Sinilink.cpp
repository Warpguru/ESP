...
static const char* TAG_SI = "SINILINK";
...
bool Sinilink::verifyDevicePresent() {
  ...
  ESP_LOGI(TAG_SI, "Product model register (0x0016) raw value: %d (0x%s)", modelValue, hexBuf);
  ...
  int fw = 0;
  readInt(FWVR, fw);
  ESP_LOGI(TAG_SI, "Detected Sinilink %s (product model: 0x%s, FW raw: %d).",
           modelName, hexBuf, fw);
  return true;
}
